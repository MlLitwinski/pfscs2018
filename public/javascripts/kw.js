...
// Koniec znaki
// Początek Funkcjonalności Generatora Dokumentów
// funkcja zapisująca obiekt
function zapiszTabele(obiekt) {
    var str = JSON.stringify(obiekt);
    sessionStorage.setItem('generatorKwIn', str);
}
// funkcja pobierająca obiekt
function odswierzTabele() {
    var pobranyObiekt = sessionStorage.getItem('generatorKwIn');
    return JSON.parse(pobranyObiekt);
}
// Przekazywanie info i budowanie obiektu do sesion storage
function generator(poz) {
    var opis = poz.getElementsByTagName("td")[0].innerHTML;
    var artKw = poz.getElementsByTagName("td")[2].innerHTML;
    var artPrd = poz.getElementsByTagName("td")[4].innerHTML;
    var kwota = poz.getElementsByTagName("td")[6].innerHTML;
    var punkty = poz.getElementsByTagName("td")[8].innerHTML;
    var kod = poz.getElementsByTagName("td")[10].innerHTML;
    var pozycja = poz.getAttribute("name");
    var obiekt = {
        "opis": opis,
        "artKw": artKw,
        "artPrd": artPrd,
...
// Koniec funkcjonalności generatora